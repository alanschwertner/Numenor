...
abstract class Numenor_Db_DataMapperAbstract {
    ...
    public function save(Numenor_Db_DomainObjectAbstract $obj) {
        if (!$obj->getId()) {
            $this->_insert($obj);
        } else {
            $this->_update($obj);
    ...
    protected function _populate($data) {
        ...
        foreach ($data as $k => $v) {
            $method = Numenor_Gerador_Nomes::getNomeMetodo('set ' . $k);
    ...
    abstract protected function _insert(Numenor_Db_DomainObjectAbstract $obj);

    abstract protected function _update(Numenor_Db_DomainObjectAbstract $obj);
}