<?php
/**
 * Numenor_CodeColorer_Geshi - Generic Syntax Highlighter
 *
 * The Numenor_CodeColorer_Geshi class for Generic Syntax Highlighting. Please refer to the
 ...
 * @copyright (C) 2004 - 2007 Nigel McNie, (C) 2007 - 2008 Benny Baumann
 */
class Numenor_CodeColorer_Geshi {
    /**#@+
     * @access private
     */
    /**
    ...
    var $error_messages = array(
        GESHI_ERROR_NO_SUCH_LANG => 'Numenor_CodeColorer_Geshi could not find the language {LANGUAGE} (using path {PATH})',
     ...
     * @since 1.0.0
     */
    function Numenor_CodeColorer_Geshi($source = '', $language = '', $path = '') {
        if (!empty($source)) {
    ...
    /**
     * Returns an error message associated with the last Numenor_CodeColorer_Geshi operation,
    ...
    function error() {
        if ($this->error) {
            ...
            return "<br /><strong>Numenor_CodeColorer_Geshi Error:</strong> $msg (code {$this->error})<br />";
        }
        return false;
    }
            ...
            if ($highlight_part) {
                ...
                for ($i = 0; $i < $length; ++$i) {
                    ...
                    if ($string_started && ($i != $next_comment_regexp_pos)) {
                        ...
                        $string = "<span$string_attributes>" . Numenor_CodeColorer_Geshi::hsc($char);
                        $start = $i + $char_len;
                        $string_open = true;
                        ...
                        do {
                            ...
                            if($simple_escape < $next_escape_regexp_pos &&
                                ...
                                $string .= "<span$escape_char_attributes>" .
                                    Numenor_CodeColorer_Geshi::hsc($this->language_data['ESCAPE_CHAR']);
...
//        $result = substr($result, 1, -1);
    ...
    function get_stylesheet($economy_mode = true) {
        ...
        if (!$economy_mode) {
            $stylesheet = "/**\n".
                " * Numenor_CodeColorer_Geshi Dynamically Generated Stylesheet\n".
                " * --------------------------------------\n".
...
if (!function_exists('geshi_highlight')) {
    ...
    function geshi_highlight($string, $language, $path = null, $return = false) {
        $geshi = new Numenor_CodeColorer_Geshi($string, $language, $path);
